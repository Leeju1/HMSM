# Lamp Return: run from the repository root.
library(Rcpp)
library(RcppArmadillo)

# 1. Input files. load the already processed file.
data_file <- "data/lamp_return/combined_lamp_return.RData"
tuning_dir <- "config/mcmc/tuning/lamp_return"
out_dir <- "outputs/lamp_return/mcmc"
load(data_file)
load(file.path(tuning_dir, "jump_kappa_lamp.RData"))
load(file.path(tuning_dir, "jump_tau_lamp.RData"))
load(file.path(tuning_dir, "jump_alpha_lamp.RData"))

# 2. Prepare the model input and item-level key-action states.
key_states <- c(3, 13, 4, 2, 11, 1, 9, 10, 8, 12, 15, 45, 5)
...
# 3. Proposal scales. Match the original scale construction, including multiplication by 0.90.
beta_scales <- array(0.09, c(2, 3, Q))
beta_scales[2, , ] <- beta_scales[2, , ] * 0.90
proposal <- list(kappa = jump_kappa, tau = jump_tau, alpha = jump_alpha,
...
input_files <- c(data_file, file.path(tuning_dir, c(
  "jump_kappa_lamp.RData", "jump_tau_lamp.RData", "jump_alpha_lamp.RData")))
